...
class SubtotalCubit
  ...
  void addToSubtotal(Product product) {
    ...
    final orderForProductIdx = state.availableProducts.indexWhere(
      (order) => order.product == product,
    );
    final orderForProduct = state.availableProducts[orderForProductIdx];
    final updatedOrderForProduct = orderForProduct.copyWith(
      amount: orderForProduct.amount - 1,
    );
    ...
    final subtotalOrderForProductIdx = state.subtotalProducts.indexWhere(
      (order) => order.product == product,
    );
    ...
    if (subtotalOrderForProductIdx == -1) {
      ...
      updatedSubtotalProducts[subtotalOrderForProductIdx] =
          updatedSubtotalProducts[subtotalOrderForProductIdx].copyWith(
  ...
  void removeFromSubtotal(Product product) {
    ...
    if (orderForProductIdx == -1) {
      ...
      updatedOrderProducts[orderForProductIdx] =
          updatedOrderProducts[orderForProductIdx].copyWith(
            amount: currentAmount + 1,
          );
    }
